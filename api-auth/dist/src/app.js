...
app.post("/auth/cadastro", (req, res) => {
    ...
    try {
        ...
        db.prepare("INSERT INTO usuarios (cpf, nome, password_hash, limite_cents) VALUES (?, ?, ?, ?)").run(cpf.trim(), nome.trim(), passwordHash, DEFAULT_LIMITE_CENTS);
...
app.post("/auth/login", (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({}, JWT_SECRET, { subject: usuario.cpf, expiresIn: JWT_TTL });
        return res.json({ token, cpf: usuario.cpf, nome: usuario.nome, expiresIn: JWT_TTL });
    }
    catch (err) {
